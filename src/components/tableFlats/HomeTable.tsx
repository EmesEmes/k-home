...
const HomeTable: React.FC<FlatTableProps> = ({
  favorites = [],
  onToggleFavorite,
  onDelete,
  onEdit,
  ...
  const buildQueryParams = (customFilters?: Partial<Filters>, customPage?: number) => {
    ...
    // Agregar paginación - usar customPage si se proporciona, sino usar pagination.page
  ...
  const fetchFlats = async (customFilters?: Partial<Filters>, customPage?: number) => {
    try {
      setLoading(true);
  ...
  const handleClean = () => {
    ...
    // Resetear a la página 1 cuando se limpian los filtros
    setPagination(prev => ({ ...prev, page: 1 }));
    fetchFlats(cleanFilters, 1);
  };
  ...
  const handlePageChange = (newPage: number) => {
    ...
    // Pasar la nueva página directamente a fetchFlats
    fetchFlats(filters, newPage);
  };
